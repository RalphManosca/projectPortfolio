export const companies = [
	{ 
		id: 1, 
		imgSRC: '',
		backgroundSRC: 'yellow',
		companyName: 'University of the East',
		role: 'role',
		skills: ['skill1', 'skill2', 'skill3', 'skill4', 'skill5'],
		workDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Minima sit a vero aut doloremque laudantium incidunt ea, voluptate porro quosducimus veritatis itaque quasi tenetur dolor, voluptates et odio inventore?', 
		achievements: ['Achievement #1', 'Achievement #2', 'Achievement #3', 'Achievement #4'], 
		learnings: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. In quod dicta neque atque, corrupti repudiandae consectetur ipsam! Repudiandae, harum nisi?'
	},
	{ 
		id: 2, 
		imgSRC: '',
		backgroundSRC: 'red',
		companyName: 'Accenture',
		role: 'role',
		skills: ['skill1', 'skill2', 'skill3', 'skill4', 'skill5'],
		workDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Minima sit a vero aut doloremque laudantium incidunt ea, voluptate porro quosducimus veritatis itaque quasi tenetur dolor, voluptates et odio inventore?', 
		achievements: ['Achievement #1', 'Achievement #2', 'Achievement #3', 'Achievement #4'], 
		learnings: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. In quod dicta neque atque, corrupti repudiandae consectetur ipsam! Repudiandae, harum nisi?'
	},
	{ 
		id: 3, 
		imgSRC: '',
		backgroundSRC: 'blue', 
		companyName: 'CTSI Logistics',
		role: 'role',
		skills: ['skill1', 'skill2', 'skill3', 'skill4', 'skill5'],
		workDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Minima sit a vero aut doloremque laudantium incidunt ea, voluptate porro quosducimus veritatis itaque quasi tenetur dolor, voluptates et odio inventore?', 
		achievements: ['Achievement #1', 'Achievement #2', 'Achievement #3', 'Achievement #4'], 
		learnings: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. In quod dicta neque atque, corrupti repudiandae consectetur ipsam! Repudiandae, harum nisi?'
	},
	{ 
		id: 4, 
		imgSRC: '',
		backgroundSRC: 'green',
		companyName: 'Medicomm Pacific',
		role: 'role',
		skills: ['skill1', 'skill2', 'skill3', 'skill4', 'skill5'],
		workDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Minima sit a vero aut doloremque laudantium incidunt ea, voluptate porro quosducimus veritatis itaque quasi tenetur dolor, voluptates et odio inventore?', 
		achievements: ['Achievement #1', 'Achievement #2', 'Achievement #3', 'Achievement #4'], 
		learnings: 'Lorem ipsum, dolor sit amet consectetur adipisicing elit. In quod dicta neque atque, corrupti repudiandae consectetur ipsam! Repudiandae, harum nisi?'
	},
]

export const projects = [
	{
		id: 1,
		name: 'The filipino Doctor',
		imageBanner: 'https://chromeunboxed.com/wp-content/uploads/2022/02/Devtools-Screenshot-1.jpg',
		projectDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque, architecto dolor recusandae doloremque voluptatem perspiciatis labore. Sed modi dignissimos enim quae officiis odio dolorem ab rem harum in ex incidunt iste nesciunt quidem debitis, ipsam, numquam, sunt corrupti aspernatur. Labore excepturi recusandae eum, eaque assumenda praesentium cum error enim maxime autem eligendi fugit. Mollitia repellendus reprehenderit facilis itaque perspiciatis. Unde?',
		link: 'https://thefilipinodoctor.com/'
	},
	{
		id: 2,
		name: 'The filipino Vet',
		imageBanner: 'https://chromeunboxed.com/wp-content/uploads/2022/02/Devtools-Screenshot-1.jpg',
		projectDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque, architecto dolor recusandae doloremque voluptatem perspiciatis labore. Sed modi dignissimos enim quae officiis odio dolorem ab rem harum in ex incidunt iste nesciunt quidem debitis, ipsam, numquam, sunt corrupti aspernatur. Labore excepturi recusandae eum, eaque assumenda praesentium cum error enim maxime autem eligendi fugit. Mollitia repellendus reprehenderit facilis itaque perspiciatis. Unde?',
		link: 'https://thefilipinovet.com/'
	},
	{
		id: 3,
		name: 'PPD Clinic Web',
		imageBanner: 'https://chromeunboxed.com/wp-content/uploads/2022/02/Devtools-Screenshot-1.jpg',
		projectDescription: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque, architecto dolor recusandae doloremque voluptatem perspiciatis labore. Sed modi dignissimos enim quae officiis odio dolorem ab rem harum in ex incidunt iste nesciunt quidem debitis, ipsam, numquam, sunt corrupti aspernatur. Labore excepturi recusandae eum, eaque assumenda praesentium cum error enim maxime autem eligendi fugit. Mollitia repellendus reprehenderit facilis itaque perspiciatis. Unde?',
		link: 'https://ppd.ph/ppd-clinic/'
	},
]